  ...
  autoFixScript = writeShellApplication {
    ...
    text = ''
      ...
      patch_bin () {
        ...
        echo "Patching Node.js of Windsurf server installation in $actual_dir..." >&2
      ...
      ''}
        ...
        ${patchELFScript}/bin/patchelf-windsurf-server "$actual_dir"
      ...
      while IFS=: read -r bins_dir bin event; do
        # A new version of the Windsurf Server is being created.
        if [[ $event == 'CREATE,ISDIR' ]]; then
          ...
          echo "Windsurf server is being installed in $actual_dir..." >&2
        ...
        # The monitored directory is deleted, e.g. when "Uninstall Windsurf Server from Host" has been run.
